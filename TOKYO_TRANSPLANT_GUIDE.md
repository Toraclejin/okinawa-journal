...
### 8.3 두 폴더 Cross-Folder 운영 룰 — 가장 중요

**핵심 원리: Read 는 자유, Write 는 본인 폴더만.**

도쿄 세션이 오키나와 폴더의 가이드·docs 를 **읽는 것은 안전** (책 빌려 읽기). 위험한 건 **두 세션이 같은 파일에 동시 write**. 이걸 룰로 차단:

#### 시나리오 분류

| # | 시나리오 | 결과 | 룰 |
|---|---|---|---|
| A | 도쿄 세션이 오키나와 docs **읽음** | ✅ 안전 | 자유 |
| B | 도쿄 세션이 오키나와 패턴 **참고해 도쿄 폴더에 반영** | ✅ 안전 | 자유 |
| C | 도쿄 세션이 **오키나와 폴더 파일 수정** | ❌ 금지 | 차단 |
| D | 오키나와 세션이 **도쿄 폴더 파일 수정** | ❌ 금지 | 차단 |
| E | 두 세션 동시에 같은 파일 write | ❌ 사고 | 1 명 즉시 멈춤 |
| F | 도쿄 작업 중 오키나와 docs 개선 필요 발견 | 🟡 명세서 | 부모 폴더 명세서로 |

#### 운영 룰 5 가지

**룰 1**: 도쿄 세션 첫 메시지에 권한 경계 명시
```
WRITE: /Users/jin/Desktop/Git/일본 여행 매거진/도쿄 여행 매거진/* (전체)
READ-ONLY: /Users/jin/Desktop/Git/일본 여행 매거진/오키나와 여행 매거진/* (참조만)
```

**룰 2**: 오키나와 세션도 도쿄 폴더 read-only.

**룰 3**: git 별도 repo. 오키나와 = `okinawa-family-journal`, 도쿄 = 새 repo (private 추천). 같은 repo 면 push 충돌 위험.

**룰 4**: 오키나와 docs 개선 제안은 명세서로:
- ❌ 도쿄 세션이 직접 오키나와 폴더 수정
- ✅ 부모 폴더 (`/일본 여행 매거진/`) 에 `TOKYO_FEEDBACK_TO_OKINAWA.md` 작성
- 사용자가 오키나와 채팅 다시 열어 적용

**룰 5**: 동시 작업 회피. 도쿄 채팅 작업 중엔 오키나와 채팅 닫음.

#### 사고 패턴 — 학습 출처

`learning.md § "두 세션이 같은 벽을 칠하면 — 도쿄 폴더 충돌 사건"` 의 실제 사고에서 도출된 룰. 이전엔 도쿄 셸이 다른 세션 덮어쓰기로 사라짐. 이번엔 위 룰로 재발 방지.
